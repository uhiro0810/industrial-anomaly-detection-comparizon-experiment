...
可視化形式は、各公開実装が異なるデフォルト出力を提供するため、手法間で完全に同一ではありません。

- **GLAD**: 入力画像、再構成画像、異常ヒートマップ、予測異常領域
- **EfficientAD**: 入力画像、オーバーレイヒートマップ、異常マップ

したがって、本リポジトリにおける定性的比較は、手法間で完全に同一のレンダリング形式を強制するのではなく、**各手法が同一入力画像上で異常領域を強調表示するか否か**に焦点を当てている。
...
左から、入力画像、オーバーレイヒートマップ、anomaly map。
