...
このリポジトリの主眼は、単なる「論文実装を動かした」ではなく、次の2点です。

- **研究再現力**  
  公開実装を読み解き、実行環境差分や依存関係の不整合を解消しながら、Colab 上で評価可能な状態まで持っていったこと
- **実装力**  
  手法ごとに異なる出力形式・評価方式を整理し、複数カテゴリの比較表、平均表、異常タイプ別分析、可視化画像まで一貫して作成できるようにしたこと
